#!/usr/bin/env python3
"""
benchmark_kd_prune.py

Pipeline:
 - Train teacher (ResNet-50)
 - Train student with KD (MobileNetV2 default)
 - Record per-epoch L1 norms of conv filters
 - HBFP-style pruning rounds (select similar filter pairs by sum-of-epoch diffs),
   run optimization regularizer to increase similarity, structured-channel prune,
   then fine-tune.
 - Benchmark (accuracy, F1, params, nonzero params, FLOPs, inference time)

References / inspiration:
 - History Based Filter Pruning (HBFP). Shabbeer Basha et al. (HBFP idea: select filters
   by history of l1 norms, optimize similarity, prune) . :contentReference[oaicite:4]{index=4}
 - Knowledge Distillation from self-supervised teacher to smaller student (KD setup).
   Kishore et al. (distillation + medical imaging context). :contentReference[oaicite:5]{index=5}
"""
...
def run_pipeline(args):
    ...
    opt_t = optim.SGD(teacher.parameters(), lr=args.lr, momentum=0.9, weight_decay=1e-4)
    ce = nn.CrossEntropyLoss()
    ...
    if args.student_arch == 'mobilenet_v2':
        ...
        # fallback: timm to create student architecture
    ...
    # We will record conv filter L1 history during KD training
    ...
    for ep in range(args.student_epochs):
        ...
        # record filter L1 history snapshot
    ...
    # prepare a small dummy loader for optimization regularizer (we can use a small batch from train)
    dummy_loader = torch.utils.data.DataLoader(next(iter(train_loader.dataset))[0].unsqueeze(0) for _ in range(1))  # fallback (not used)
    # instead, create a small subset dataloader
    small_subset = torch.utils.data.Subset(train_loader.dataset, list(range(min(256, len(train_loader.dataset)))))
    dummy_loader = torch.utils.data.DataLoader(small_subset, batch_size=args.batch_size, shuffle=True, num_workers=0)
